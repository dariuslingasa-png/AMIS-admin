            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <p class="mt-2 max-w-2xl text-sm font-medium leading-6 text-slate-300">
                        Monitor active administrative login sessions, client IP addresses, browser types, devices, and session duration.
                    </p>
                </div>
            </div>
        </section>

        <x-card title="Authentication Sessions Log" subtitle="Track user sign-ins and logout times">
            <div class="px-4 py-4 sm:px-6 border-b border-slate-200/60 bg-slate-50/50">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-end">
                    <!-- Search Input Form -->
                    <form method="GET" action="{{ route('admin.admins.login-activity') }}" class="flex items-center gap-2">
                        <div class="relative w-full sm:w-[320px]">
                            <i data-lucide="search" class="pointer-events-none absolute left-3.5 top-3.5 h-4 w-4 text-slate-400"></i>
                            <input type="text" name="search" value="{{ $search }}" placeholder="Search user, email, IP..." class="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-10 text-xs font-semibold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100">
                            @if(filled($search))
                                <a href="{{ route('admin.admins.login-activity') }}" class="absolute right-3 top-3.5 text-slate-400 hover:text-slate-600 transition" title="Clear search">
