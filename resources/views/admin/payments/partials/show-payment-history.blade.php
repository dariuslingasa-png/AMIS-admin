                                    ...
                                    <td class="px-4 py-4 text-right font-bold">
                                        @if (strtolower($p->status) === 'pending')
                                            <div class="flex items-center justify-end gap-2">
                                                <button type="button" @click="approveModal = true; currentPayment = {{ $p->id }}; currentInvoice = '{{ $pInvoiceNo }}'; predictedOr = '{{ $predictedOr }}'; financeAmount = '{{ $p->amount }}'; isSubmitting = false; financeMethod = '{{ in_array(strtolower($p->method), ['remittance', 'gcash', 'bdo', 'maya', 'cash']) ? strtolower($p->method) : 'other' }}'; remittanceSource = ''; financePaymentDate = '{{ $p->paid_at?->format('Y-m-d') ?: ($p->created_at?->format('Y-m-d') ?: now()->format('Y-m-d')) }}'; financeReferenceNo = '{{ $p->reference_no }}'; document.getElementById('approve-form').action = '{{ route('admin.payments.verify', $p) }}';" class="btn-premium btn-approve">
                                                    Verify
                                                </button>
