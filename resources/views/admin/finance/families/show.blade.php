                ...
                @foreach ($family->enrollmentApplicants as $applicant)
                    @if ($applicant->student?->account)
                        ...
                        <div
                            ...
                            class="cursor-pointer rounded-2xl border bg-white p-5 shadow-2xs hover:shadow-md transition duration-200 flex flex-col justify-between"
                            :class="selectedStudentId === '{{ $sId }}' ? 'border-slate-900 ring-2 ring-slate-900 shadow-md' : 'border-slate-200/90 hover:border-slate-300'"
                        >
                            <div>
                                {{-- CARD TOP ROW --}}
                                ...
                                {{-- FINANCIAL STATS ROW --}}
                                <div class="mt-4 rounded-xl bg-slate-50/90 border border-slate-100 p-3.5">
                            ...
                            {{-- CARD ACTION FOOTER --}}
                            <div class="mt-3.5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-bold" :class="selectedStudentId === '{{ $sId }}' ? 'text-slate-900' : 'text-slate-600'">
            ...
            @foreach ($family->enrollmentApplicants as $applicant)
                @if ($applicant->student?->account)
                    ...
                    >
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 pb-4">
                            <div>
                                <div class="flex items-center gap-2 flex-wrap">
                                    <span class="rounded-full bg-slate-900 px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider text-white">Student Account</span>
                                    <h3 class="text-xl font-black tracking-tight text-slate-900">{{ mb_strtoupper($sName) }}</h3>
                                    <span class="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-700">{{ $sGrade }}</span>
                            ...
                            <div class="flex items-center gap-3">
                                <div class="text-right">
                                    ...
                                    <span class="text-xl font-black text-slate-900">₱{{ number_format($sBalance, 2) }}</span>
                                </div>
                        ...
                        <div class="mt-6 grid gap-6 lg:grid-cols-[1.2fr_0.8fr] items-start">
                            {{-- LEFT COLUMN: MONTHLY PAYMENT SCHEDULE --}}
                            <div>
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <span class="text-[11px] font-bold text-slate-400">FIFO Month Order</span>
                                </div>

                                <div class="divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white overflow-hidden shadow-2xs">
                                    <div class="grid grid-cols-[1.5fr_1fr_1fr_1fr_auto] gap-2 px-4 py-2.5 bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500 border-b border-slate-200">
                                        <span>Month</span>
                                        <span class="text-right">Monthly Fee</span>
                                        <span class="text-right">Paid</span>
                                        <span class="text-right">Balance</span>
                                        <span class="text-center w-20">Status</span>
                                    </div>
                                    @forelse ($studentSchedule as $m)
                                        ...
                                        @endphp
                                        <div class="grid grid-cols-[1.5fr_1fr_1fr_1fr_auto] gap-2 px-4 py-3 items-center text-xs hover:bg-slate-50/80 transition">
                                            <div class="font-bold text-slate-900 truncate">
                                                {{ $m->month }}
                                            </div>
                                            <div class="text-right font-medium text-slate-600">
                                            ...
                                            <div class="text-center w-20">
                                                <span class="inline-block rounded px-2 py-0.5 text-[9px] font-black uppercase {{ $stClass }}">
                                                    {{ $m->status }}
                                                </span>
                                            </div>
                                        </div>
                                    @empty
                                        <div class="p-6 text-center text-xs text-slate-400">No monthly schedule generated.</div>
                                    @endforelse
                                </div>
                            </div>

                            {{-- RIGHT COLUMN: STATEMENT OF ACCOUNT WORKFLOWS (OPTIONS A & B) --}}
                            <div class="space-y-5">
                                {{-- OPTION A: FINANCE-UPLOADED MANUAL SOA --}}
                                <div class="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
                                    <div class="flex items-center justify-between mb-3">
                                        <div>
                                            <span class="rounded-md bg-slate-900 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-white">Option 1</span>
                                            <h5 class="mt-1 text-xs font-bold text-slate-900">Finance-Uploaded Manual SOA</h5>
                                        </div>
                                        <button type="button" @click="openUpload({{ Js::from($studentData) }})" class="inline-flex items-center gap-1 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 px-3 py-1.5 rounded-lg shadow-2xs transition">
                                        ...
                                        <div class="rounded-xl border border-slate-200 bg-white p-3.5 shadow-2xs">
                                            <div class="flex items-center justify-between gap-2">
                                                <div class="flex items-center gap-2">
                                                    <span class="rounded-md bg-emerald-100 text-emerald-800 px-2 py-0.5 text-[10px] font-black uppercase">{{ $latestManualSoa->billing_month }}</span>
                                            ...
                                            @if ($latestManualSoa->remarks)
                                                <p class="mt-2 text-[11px] text-slate-500 italic bg-slate-50 rounded-md px-2.5 py-1.5 border border-slate-100">
                                                    "{{ $latestManualSoa->remarks }}"
                                                </p>
                                            @endif

                                            <div class="mt-3 flex items-center gap-2">
                                                <button type="button" @click="openPreview('{{ route('admin.finance.manual-soa.view', $latestManualSoa) }}', '{{ $latestManualSoa->student_name }} · {{ $latestManualSoa->billing_month }} SOA', {{ $latestManualSoa->is_pdf ? 'true' : 'false' }})" class="flex-1 rounded-lg bg-slate-900 px-3 py-1.5 text-center text-xs font-bold text-white hover:bg-slate-800 shadow-2xs transition">
                                                    View Document
                                                </button>
                                                <a href="{{ route('admin.finance.manual-soa.download', $latestManualSoa) }}" class="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-center text-xs font-bold text-slate-700 hover:bg-slate-50 shadow-2xs transition" title="Download Document">
                                                ...
                                                @if ($studentSoaList->count() > 1)
                                                    <button type="button" @click="openHistory({{ Js::from($studentData) }}, {{ Js::from($studentSoaList) }})" class="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-50 shadow-2xs transition">
                                    ...
                                    @else
                                        <div class="rounded-xl border border-dashed border-slate-200 bg-white p-4 text-center">
                                            ...
                                            <button type="button" @click="openUpload({{ Js::from($studentData) }})" class="mt-2 inline-flex items-center gap-1.5 rounded-xl bg-slate-900 px-3.5 py-1.5 text-xs font-bold text-white hover:bg-slate-800 shadow-2xs transition">
                                                <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4.5v15m7.5-7.5h-15"/></svg>
                                ...
                                <div class="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
                                    <div class="flex items-center justify-between mb-2">
                                        <div>
                                            <span class="rounded-md bg-emerald-700 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-white">Option 2</span>
                                            <h5 class="mt-1 text-xs font-bold text-slate-900">Official School Statement of Account (SY 2026-2027)</h5>
                                        </div>
                                    </div>
                                    <p class="text-[11px] text-slate-500 mb-3">
                                        Official template format with Arabic school branding, DepED permits, Quranic ayah, and complete fee assessment ledger.
                                    </p>
                                    <a
