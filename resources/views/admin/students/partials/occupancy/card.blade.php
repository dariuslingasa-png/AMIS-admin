                        ...
                        <tr class="hover:bg-slate-50/40 transition" style="border-bottom: {{ $loop->last ? 'none' : '1px solid #f1f5f9' }};">
                            ...
                            <td class="py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <button type="button" @click="if (window.AMIS_OCCUPANCY) { window.AMIS_OCCUPANCY.openJsonModal = true; window.AMIS_OCCUPANCY.selectedTargetSection = '{{ $section->id }}'; window.AMIS_OCCUPANCY.selectedGradeLevel = '{{ $gradeLevel }}'; } window.dispatchEvent(new CustomEvent('open-json-sync', { detail: { sectionId: '{{ $section->id }}', gradeLevel: '{{ $gradeLevel }}' } }))" class="h-7 w-7 rounded-lg bg-teal-50 text-teal-700 border border-teal-100 hover:bg-teal-100 flex items-center justify-center transition active:scale-[0.95] cursor-pointer" title="JSON Batch Sync for {{ $sectionDisplayName }}">
                                        <i data-lucide="file-json" class="h-3.5 w-3.5 pointer-events-none"></i>
                                    </button>
