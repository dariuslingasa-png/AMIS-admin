<x-admin-layout title="Create Announcement">
    ...
    <div class="max-w-3xl mx-auto">
        <div class="mb-6 flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">Publish a new post or event update to the school website.</p>
            </div>
        </div>

        <!-- Form Card -->
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/70 dark:border-gray-700/50 p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.website.announcements.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" id="title" name="title" value="{{ old('title') }}" required placeholder="e.g. Enrollment Opening for Secondary School 2026" class="w-full rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="category_select" onchange="toggleCustomCategory(this)" class="w-full rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                        <option value="AMIS News" @selected(old('category', 'AMIS News') === 'AMIS News')>AMIS News</option>
                        <option value="Event" @selected(old('category') === 'Event')>Event</option>
                        <option value="Announcement" @selected(old('category') === 'Announcement')>Announcement</option>
                        <option value="Important Advisory" @selected(old('category') === 'Important Advisory')>Important Advisory</option>
                        <option value="Good News" @selected(old('category') === 'Good News')>Good News</option>
                        <option value="custom" @selected(old('category') && !in_array(old('category'), ['AMIS News', 'Event', 'Announcement', 'Important Advisory', 'Good News']))>Custom Category...</option>
                    </select>
                    
                    @php
                        $isCustom = old('category') && !in_array(old('category'), ['AMIS News', 'Event', 'Announcement', 'Important Advisory', 'Good News']);
                    @endphp
                    <input type="text" id="category" name="category" value="{{ old('category', 'AMIS News') }}" required class="w-full rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent mt-2 {{ $isCustom ? '' : 'hidden' }}" placeholder="Type custom category name...">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="priority" name="priority" required class="w-full rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 text-sm text-slate-850 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                        <option value="normal" @selected(old('priority', 'normal') === 'normal')>Normal Priority</option>
                        <option value="high" @selected(old('priority') === 'high')>High / Featured</option>
                    </select>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="author" name="author" value="{{ old('author', 'AMIS Admin') }}" required placeholder="e.g. AMIS Registrar" class="w-full rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="datetime-local" id="publish_date" name="publish_date" value="{{ old('publish_date', now()->format('Y-m-d\TH:i')) }}" required class="w-full rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                </div>
            </div>

            <!-- Event-specific details (only visible if Category is Event) -->
            <div id="event_details_container" class="p-5 bg-teal-50/50 dark:bg-teal-950/10 border border-teal-100 dark:border-teal-900/50 rounded-2xl space-y-4 {{ old('category') === 'Event' ? '' : 'hidden' }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" id="event_dates" name="event_dates" value="{{ old('event_dates') }}" placeholder="e.g. August 21–24, 2026 or Every Friday" class="w-full rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" id="event_venue" name="event_venue" value="{{ old('event_venue') }}" placeholder="e.g. Berjaya Times Square Hotel, Kuala Lumpur or Zoom" class="w-full rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                    </div>
                </div>
                
                <!-- Online Class / Virtual Toggle -->
                <div class="flex items-center gap-2 pt-2">
                    <input type="checkbox" id="is_online" name="is_online" value="1" @checked(old('is_online', false)) class="rounded border-slate-300 text-teal-600 focus:ring-teal-500 h-4 w-4">
            ...
            <!-- Cover Image File -->
            <div class="space-y-1.5">
                <label for="image_files" class="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Cover Images / Posters (Optional, Multiple Allowed)</label>
