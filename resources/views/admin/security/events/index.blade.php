                    ...
                    <div class="flex items-center gap-2">
                        <form method="GET" action="{{ route('admin.security-workspace.events.index') }}" class="flex items-center gap-2">
                            <div class="relative w-full sm:w-[320px]">
                                ...
                                <input type="text" name="search" value="{{ $search }}" placeholder="Search user or message..." class="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-10 text-xs font-semibold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-rose-400 focus:ring-4 focus:ring-rose-100">
                                @if(filled($search))
