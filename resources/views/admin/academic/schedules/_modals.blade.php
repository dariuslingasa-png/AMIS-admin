...
<div class="admin-modal-overlay flex items-center justify-center fixed inset-0 z-50 bg-slate-955/40 backdrop-blur-xs p-4"
     x-show="addModal" x-cloak x-transition @click.self="addModal = false">
    <form method="POST" action="{{ route('admin.academic.schedules.store') }}" class="admin-modal-card bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 flex flex-col gap-4 border border-slate-200" @submit="isSaving = true">
        @csrf
...
<div class="admin-modal-overlay flex items-center justify-center fixed inset-0 z-50 bg-slate-955/40 backdrop-blur-xs p-4"
    ...
    <form method="POST" :action="editAction" class="admin-modal-card bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 flex flex-col gap-4 border border-slate-200" @submit="isSaving = true">
        @csrf
        @method('PATCH')
        ...
        <div class="admin-modal-header border-b border-slate-100 pb-3 flex items-center justify-between">
            <div>
                <span class="admin-modal-title text-base font-extrabold text-slate-955">Edit Schedule</span>
...
<div class="admin-modal-overlay flex items-center justify-center fixed inset-0 z-50 bg-slate-955/40 backdrop-blur-xs p-4"
    ...
    <form method="POST" :action="deleteAction" class="admin-modal-card bg-white rounded-2xl shadow-xl w-full max-w-md p-6 flex flex-col gap-4 border border-slate-200" @submit="isDeleting = true">
        ...
        @method('DELETE')
        <div class="admin-modal-header border-b border-slate-100 pb-3 flex items-center justify-between">
            <span class="admin-modal-title text-base font-extrabold text-slate-955">Delete Schedule</span>
