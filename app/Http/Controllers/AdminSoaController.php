...
class AdminSoaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($s, $sl) {
                $q->whereHas('student', fn ($sq) => $sq->where('student_number', 'like', "%{$s}%")
                    ->orWhereHas('applicant', fn ($a) => $a->whereRaw('LOWER(first_name) LIKE ?', ["%{$sl}%"])
        ...
        $gradeLevels = StudentAccount::select('grade_level')
    ...
    public function addPayment(Request $request, StudentAccount $account)
    {
        ...
        if (! empty($validated['soa_monthly_billing_id'])) {
            SoaMonthlyBilling::find($validated['soa_monthly_billing_id'])
