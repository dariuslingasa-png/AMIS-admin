...
final class FinalPaymentAdvisoryTestMail extends Mailable
{
    ...
    public function __construct(
        public string $pageOnePath,
        public string $pageTwoPath,
        public string $logoPath,
        public string $dispatchRef,
    ) {
        ...
        $this->withSymfonyMessage(function (\Symfony\Component\Mime\Email $message): void {
            ...
            $message->embedFromPath($this->pageOnePath, 'memorandum-page-1', 'image/png');
            $message->embedFromPath($this->pageTwoPath, 'memorandum-page-2', 'image/jpeg');
        });
    }
    ...
    {
        return new Envelope(
            from: new Address($this->fromAddress, 'Al Munawwara Islamic School'),
            replyTo: [new Address('amisfinance2324@gmail.com', 'AMIS Finance')],
            subject: "[TEST] ADVISORY — Final Payment Reminder Before the First Term Examination [{$this->dispatchRef}]",
        );
    }
    ...
    public function headers(): Headers
    {
        ...
        return new Headers(
            ...
            text: [
                ...
                'X-AMIS-Advisory-Test' => $this->dispatchRef,
            ],
        );
    }
    ...
    {
        return [
            Attachment::fromPath($this->pageOnePath)
                ->as('AMIS-Advisory-SM-2627-018-Page-1.png')
                ->withMime('image/png'),
            Attachment::fromPath($this->pageTwoPath)
      ...
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px;background:#ffffff;border:1px solid #dbe5df;border-radius:14px;overflow:hidden;">
        ...
        <tr>
          <td style="padding:4px 20px 24px;background:#ffffff;text-align:center;">
            ...
            <img src="cid:memorandum-page-1" alt="Payment Advisory Page 1" width="624" style="display:block;width:100%;max-width:624px;height:auto;margin:0 auto 18px;border:1px solid #d1d5db;border-radius:8px;">
            <img src="cid:memorandum-page-2" alt="Payment Advisory Page 2" width="624" style="display:block;width:100%;max-width:624px;height:auto;margin:0 auto;border:1px solid #d1d5db;border-radius:8px;">
          </td>
        </tr>
        <tr>
          <td style="background:#f0fdf4;border-top:1px solid #d1fae5;padding:16px 28px;text-align:center;color:#4b5563;font-size:12px;line-height:1.5;">
            This test email includes the two-page payment advisory as attachments.<br>
...
$target = 'zhairel.lingasa@gmail.com';
$pageOne = dirname(__DIR__).'/storage/app/private/payment-advisory-20260827/MEMORANDUM1.png';
$pageTwo = dirname(__DIR__).'/storage/app/private/payment-advisory-20260827/MEMORANDUM2.jpg';
$logo = dirname(__DIR__).'/public/images/AMIS_Logo_email.png';
$dispatchRef = 'TEST-'.now()->format('Ymd-His');

foreach ([$pageOne, $pageTwo, $logo] as $attachmentPath) {
    if (! is_file($attachmentPath) || filesize($attachmentPath) === 0) {
        throw new RuntimeException("Required memorandum attachment is missing: {$attachmentPath}");
    }
}

/** @var SmartSmtpRotatorService $rotator */
$rotator = app(SmartSmtpRotatorService::class);
$errors = [];

foreach ($rotator->getMailerPool() as $mailer) {
    $username = strtolower(trim((string) config("mail.mailers.{$mailer}.username")));

    if ($username === '' || $username === strtolower($target)) {
        continue;
    }

    if ($rotator->getDailyCount($mailer) >= 450) {
        continue;
    }

    try {
        $mailable = new FinalPaymentAdvisoryTestMail($pageOne, $pageTwo, $logo, $dispatchRef);
        $mailable->fromAddress = $username;

        Mail::mailer($mailer)->to($target)->send($mailable);
        $rotator->incrementDailyCount($mailer);
        ...
        echo json_encode([
            ...
            'attachments' => [basename($pageOne), basename($pageTwo)],
        ], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES).PHP_EOL;

        exit(0);
    } catch (Throwable $exception) {
        $errors[$mailer] = substr($exception->getMessage(), 0, 240);
    }
}
